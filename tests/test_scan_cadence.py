...
class AdaptiveScanCadenceTests(unittest.TestCase):
    def test_risk_on_uses_fast_interval(self):
        with patch("src.main.settings.SCAN_INTERVAL_FAST_SECONDS", 60), \
             ...
             patch("src.main.settings.SCAN_INTERVAL_EXTENDED_SECONDS", 300), \
             patch("src.main.settings.SCAN_INTERVAL_OVERNIGHT_SECONDS", 600):
    ...
    def test_extended_session_enforces_extended_floor(self):
        with patch("src.main.settings.SCAN_INTERVAL_FAST_SECONDS", 60), \
             patch("src.main.settings.SCAN_INTERVAL_SLOW_SECONDS", 300), \
             patch("src.main.settings.SCAN_INTERVAL_SECONDS", 180), \
             patch("src.main.settings.SCAN_INTERVAL_EXTENDED_SECONDS", 300), \
             patch("src.main.settings.SCAN_INTERVAL_OVERNIGHT_SECONDS", 600):
            self.assertEqual(TradingBot._determine_scan_interval("risk_on", session="extended"), 300)
            self.assertEqual(TradingBot._determine_scan_interval("mixed", session="extended"), 300)

    def test_overnight_session_enforces_overnight_floor(self):
        with patch("src.main.settings.SCAN_INTERVAL_FAST_SECONDS", 60), \
             patch("src.main.settings.SCAN_INTERVAL_SLOW_SECONDS", 300), \
             patch("src.main.settings.SCAN_INTERVAL_SECONDS", 180), \
             patch("src.main.settings.SCAN_INTERVAL_EXTENDED_SECONDS", 300), \
             patch("src.main.settings.SCAN_INTERVAL_OVERNIGHT_SECONDS", 600):
            self.assertEqual(TradingBot._determine_scan_interval("risk_on", session="overnight"), 600)
            self.assertEqual(TradingBot._determine_scan_interval("choppy", session="overnight"), 600)
