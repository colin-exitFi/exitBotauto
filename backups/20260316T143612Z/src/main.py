...
class TradingBot:
    ...
    async def _provider_health_check(self) -> Dict[str, Dict]:
        ...
        if len(non_rate_limit_failures) >= 2:
            logger.critical(
                ...
                "Jury reliability is compromised until providers recover."
            )
        return status
    ...
    async def _process_breakout_queue(self):
        ...
        if not guardrails.get("allow_new_entries", True):
            logger.warning(
                f"⚡ FAST-PATH blocked by operating guardrails: {','.join(guardrails.get('reasons', []))}"
            )
            return
    ...
    async def _execute_fast_path_scout_entry(self, candidate: dict):
        ...
        try:
            # Fast-path breakout is regular-hours only. Extended-hours liquidity/noise
            # produced repeated ghost/pending states in forensic logs.
            regular_hours = None
            if self.extended_guard:
                regular_hours = self.extended_guard.is_regular_hours()
            elif self.entry_manager:
                regular_hours = not self.entry_manager.is_extended_hours()
            if regular_hours is False:
                logger.info(f"⚡ FAST-PATH blocked outside regular hours: {symbol}")
                return
