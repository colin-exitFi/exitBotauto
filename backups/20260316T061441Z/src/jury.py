...
async def deliberate(symbol: str, price: float, briefs: Dict, signals_data: Dict = None) -> JuryVerdict:
    ...
    try:
        ...
        # NOTE: max_size_pct can be 0.0, which must still be enforced.
        risk_max = risk_brief.get("max_size_pct") if risk_brief else None
        if risk_max is not None:
            risk_max_f = float(risk_max)
            verdict.size_pct = min(verdict.size_pct, risk_max_f)
            if risk_max_f <= 0.0 and verdict.decision in ("BUY", "SHORT"):
                logger.warning(
                    f"🛡️ Risk max_size_pct=0.0 for {symbol} — forcing SKIP "
                    f"(was {verdict.decision})"
                )
