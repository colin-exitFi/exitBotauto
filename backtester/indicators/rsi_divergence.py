...
class RSIDivergenceIndicator(BaseIndicator):
    ...
    def generate_signals(self, df: pd.DataFrame):
        ...
        bullish_div = (close < close.shift(1).rolling(5).min()) & (rsi_series > rsi_series.shift(1).rolling(5).min())
        bearish_div = (close > close.shift(1).rolling(5).max()) & (rsi_series < rsi_series.shift(1).rolling(5).max())
        entries = crossed_above(rsi_series, pd.Series(30.0, index=df.index)) | bullish_div.fillna(False)
        exits = crossed_below(rsi_series, pd.Series(70.0, index=df.index)) | bearish_div.fillna(False)
